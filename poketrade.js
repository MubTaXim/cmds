...
if(!pokedb[event.threadID].usdata.hasOwnProperty(event.senderID)) return message.send(" You don't have any pokemons yet.")

      if(mentionv.length == 0) return message.reply("baka mention someone")
if(!pokedb[event.threadID].usdata.hasOwnProperty(mentionk[0])) return message.send("Opposition don't have any pokemons yet.")
...
if(!pokedb[event.threadID].usdata[event.senderID].includes(arr[0].trim().toLowerCase())) return message.send(`You don't have  ${arr[0]} in your pokedex`)

      if(!pokedb[event.threadID].usdata[mentionk[0]].includes(arr[1].trim().toLowerCase())) return message.send(`Opposition don't have  ${arr[1]} in their pokedex`)
